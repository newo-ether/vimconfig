...
function! s:get_bufline_non_memoized(window, ...)
  ...
  if g:show_file_explorer[tabpagenr()]
      ...
      let file_explorer_text_len = len(file_explorer_text) < file_explorer_width - 2 ? len(file_explorer_text) : file_explorer_width - 2
      if len(file_explorer_text) > file_explorer_text_len
          let file_explorer_text = file_explorer_text[0: file_explorer_text_len - 1 - 3]
          let file_explorer_text = add(file_explorer_text, '...')
      endif

      let file_explorer_prefix_width = (file_explorer_width - file_explorer_text_len) / 2
      let file_explorer_suffix_width = file_explorer_width - file_explorer_prefix_width - file_explorer_text_len
      for n in range(file_explorer_prefix_width)
          let file_explorer_label = add(file_explorer_label, ' ')
      endfor
      let file_explorer_label = file_explorer_label + file_explorer_text
      for n in range(file_explorer_suffix_width)
          let file_explorer_label = add(file_explorer_label, ' ')
      endfor
      let file_explorer_label = add(file_explorer_label, '|')
      let file_explorer_label = join(file_explorer_label, '')
      let file_explorer_element = {
          \'label': file_explorer_label,
          \'highlight': 0,
          \'type': 'sep',
          \}
      call add(line, file_explorer_element)
  endif
